...
class FDKDataProvider:
    ...
    async def get_object(self, object_id: str) -> FDKObject | None:
    ...
    async def _fetch_from_api(self, object_id: str) -> FDKObject | None:
    ...
    def _matches_criteria(self, obj: FDKObject, criteria: FDKSearchCriteria) -> bool:
        ...
        if (
            criteria.object_type
            and criteria.object_type.upper() != obj.object_type.upper()
        ):
            return False
    ...
    def extract_object_id_from_filename(self, filename: str) -> str | None:
    ...
    async def analyze_references(
        self, object_id: str, max_depth: int = 3
    ) -> FDKReferenceAnalysis:
        ...
        analysis.reference_count = len(analysis.referenced_by) + len(
            analysis.references_to
        )
